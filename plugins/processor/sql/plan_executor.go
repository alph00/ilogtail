...
func (p *ProcessorSql) evaluteFunc(node Expr_, content *LogContentType) (string, error) {
	...
	for _, item := range node.(*Func_).Param {
		...
		case *Func_: // to do: check if the ressult is correct
			{
...
// short-circuit optimization短路优化
// func (p *ProcessorSql) evaluateConditionPairsParallel(node *Cond_, content *LogContentType) bool {
// 	switch node.Comp.Name {
// 	case And:
// 		{
...
// 			go func() {
// 				chanOne <- p.traverse(node.Lexpr.(*Cond_), content)
// 			}()
// 			go func() {
// 				chanTwo <- p.traverse(node.Rexpr.(*Cond_), content)
// 			}()
// 			select {
// 			case res := <-chanOne:
// 				return res
...
// 	case Or:
// 		{
// 			if p.traverse(node.Lexpr.(*Cond_), content) {
// 				return true
// 			}
// 			if p.traverse(node.Rexpr.(*Cond_), content) {
// 				return true
// 			}
// 			return false
// 		}
